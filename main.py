...
params = {
    "random_state": 1337,
    "C_lower": 1e-9,  # lower bound for C search
    ...
    "n_iters": 300,  # number of iterations of Bayes search
...
if __name__ == "__main__":
    ...
    # calculating weight of evidence on training set
    woe = ce.woe.WOEEncoder(cols=X_train.columns.tolist(), drop_invariant=True)
    ...
    # applying weight of evidence of validation and test sets
    X_val_woe = woe.transform(X_val)
    y_val_pred = model.predict(X_val_woe)
    ...
    log_path, fig_path = time_path + ".txt", time_path + ".pdf"
    with open(log_path, "w") as f:
        best_params = \
            report_performance(optimizer,
                               X_train_woe, y_train,
                               callbacks=[VerboseCallback(params.n_iters),
                                          DeadlineStopper(params.n_iters * 3)])
